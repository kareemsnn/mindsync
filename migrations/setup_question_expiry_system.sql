...
-- Step 2: Store the API endpoint in the vault (this is idempotent)
-- If the secret already exists, this will update its value
SELECT vault.create_secret(
  'api_endpoint', 
  'https://mindsync-v1-cb054f8af12c.herokuapp.com'
);

-- Step 3: Create or replace the process_expired_questions function
...
-- Step 4: Schedule the job (first remove if it already exists)
SELECT cron.unschedule('process-expired-questions');

-- Step 5: Schedule the job to run every 5 minutes
...
-- Step 6: Verify the job is scheduled (uncomment to check)
